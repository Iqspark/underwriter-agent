...
| **3** | **Event-driven runtime + durable workflow + async HITL** (broker, Temporal, retries/DLQ/outbox, state machine) | Handles enrichment/LLM latency, spikes, and human-in-the-loop. **✅ Lean tier built** ([ADR-0010](adr/0010-event-driven-runtime.md)): async case lifecycle + durable state machine + outbox + idempotency + retries→DLQ + 202/poll API; Kafka/Temporal deferred. |
| **4** | **MCP enrichment** (geocoding + peril/**crime** + property data) | Biggest *accuracy* win; needs the async runtime. **✅ Offline baseline built** ([ADR-0026](adr/0026-mcp-enrichment.md)): `EnrichmentProvider` tool boundary + cache + degrade-to-floor + advisory peril findings; MCP tool servers plug in behind the seam. |
| **5** | **Multimodal intake + drafting** — incl. **semantic feature extraction** ([ADR-0021](adr/0021-semantic-feature-extraction.md)) | Biggest *time-savings* for people; also lifts accuracy by feeding extracted features to the model. **✅ Built:** semantic features (advisory-first, ADR-0021) + drafting ([ADR-0028](adr/0028-phase5-intake-drafting.md)); multimodal/vision intake + feeding features into k-NN deferred. |
...
| **7** | **Dashboards + AI-ops + data flywheel** (Grafana + UW performance dashboard, evals, drift, outcome feedback) | Required before widening autonomy. **✅ Baseline built** ([ADR-0027](adr/0027-dashboards-flywheel.md)): realized-outcome capture + `OutcomeRecorded`, KPI dashboard (`GET /dashboard`), business metrics; Grafana/warehouse + book-refresh jobs pending. |
