...
import com.iqspark.underwriter.review.ReviewResult;
...
public class DecisionOrchestrator {
    ...
    public Decision decide(Submission submission) {
        ...
        Reviewed reviewed = applyReviewer(ctx, submission, outcome, findings, learned, rationale);
    ...
    private String generateRationale(UnderwritingContext ctx, Submission submission,
        ...
        String reasonerUsed;
        try {
            rationale = primaryReasoner.summarize(submission, outcome, findings, learned,
                    ctx.indicativePremium(), ctx.retrievedSources());
            ...
            rationale = templateReasoner.summarize(submission, outcome, findings, learned,
                    ctx.indicativePremium(), ctx.retrievedSources());
    ...
    /** Run the Reviewer agent (if present); on a rationale defect, allow one LLM re-draft. Advisory. */
    private Reviewed applyReviewer(UnderwritingContext ctx, Submission submission, DecisionOutcome outcome,
                                   List<Finding> findings, LearnedAssessment learned, String rationale) {
        if (reviewer == null) {
            return new Reviewed(rationale, List.of());
        }
        ReviewResult result = reviewer.review(outcome, findings, rationale);
        String finalRationale = rationale;

        boolean canRedraft = !primaryReasoner.name().equals(templateReasoner.name());
        if (result.rationaleDefect() && canRedraft) {
            try {
                String redraft = primaryReasoner.summarize(submission, outcome, findings, learned,
                        ctx.indicativePremium(), ctx.retrievedSources());
                ReviewResult reReview = reviewer.review(outcome, findings, redraft);
                finalRationale = redraft;
                result = reReview;
                ctx.audit("ReviewerAgent", "Rationale re-drafted after a review flag.");
            } catch (LlmReasoningException e) {
                log.warn("Rationale re-draft failed; keeping original: {}", e.getMessage());
            }
        }

        ctx.audit("ReviewerAgent", result.flags().isEmpty()
                ? "Review complete: coherent, no flags."
                : "Review complete: %d flag(s)%s.".formatted(result.flags().size(),
                        result.humanReviewRecommended() ? " — human review recommended" : ""));
        return new Reviewed(finalRationale, result.flags());
    }

    private record Reviewed(String rationale, List<ReviewFlag> flags) {}
